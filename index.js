...
function discordprofile(id) {
    ...
    xhr.onload = async function () {
      if (this.status == 200) {
        ...
        const discord_data = discord_data_raw.data;
        let avatar_url = `https://cdn.discordapp.com/avatars/${discord_data.discord_user.id}/${discord_data.discord_user.avatar}?size=4096`;
        if (discord_data.discord_user.discriminator == "0")
          {var disusername = `<a href="https://discord.com/users/${discord_data.discord_user.id}" style="text-decoration: unset;color: unset;"> @${discord_data.discord_user.display_name} </a>`} 
            else
        ...
        document.getElementById('nameplate-switch').onclick = function () {
          if (discord_data.discord_user.collectibles?.nameplate) {
              ...
              document.getElementsByClassName('card')[0].insertAdjacentHTML('afterbegin', `
                <video src="https://cdn.discordapp.com/assets/collectibles/${discord_data.discord_user.collectibles.nameplate.asset}asset.webm" loop="" autoplay="" style="
                position: absolute;
                right: 0;
                top: 0;
                width: ${Kard.width}px;
        ...
        .then(data => {
            ...
            if (parser.parseFromString(data, "text/html")?.querySelectorAll('.banner img')[0]?.src == undefined) {
              // Banner from usrbg
              var bg = `https://usrbg.is-hardly.online/usrbg/v2/${discord_data.discord_user.id}?9807=&size=1024&format=gif`;
            } else {
              // Banner from Discord
      ...
      /*switch (discord_data.activities[0].id) {
      }*/}
      ...
      switch (discord_data.discord_status) {
          case 'online':
        ...
        for (p of discord_data?.activities) {
          if (p.platform == 'xbox') {
        ...
        discord_data.active_on_discord_web ? document.getElementById('status').insertAdjacentHTML('beforeend', web) : undefined;
        discord_data.active_on_discord_mobile ? document.getElementById('status').insertAdjacentHTML('beforeend', mobile) : undefined;
        discord_data.active_on_discord_desktop ? document.getElementById('status').insertAdjacentHTML('beforeend', pc) : undefined;
      //if (typeof discord_data.activities[0] !== 'undefined')
        switch (discord_data.activities[0]?.type) {
          case 0:
          activity("In game: ");
          break;
          case 1:
